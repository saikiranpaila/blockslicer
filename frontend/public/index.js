...
const MENU_LEADERBOARD = Symbol('MENU_LEADERBOARD')
const MENU_LOGIN = Symbol('MENU_LOGIN')
...
const isNewHighScore = () => state.game.score > getHighScore();
...
const menuLeaderBoardNode = $('.menu--leaderboard')
const menuLoginNode = $('.menu--login')
...
function renderMenus() {
	...
	hideMenu(menuLeaderBoardNode);
	hideMenu(menuLoginNode);
	...
	switch (state.menus.active) {
		...
		case MENU_LEADERBOARD:
			showMenu(menuLeaderBoardNode);
			break;
		case MENU_LOGIN:
			showMenu(menuLoginNode);
			break;
	}
...
handleClick($('.leaderboard-btn'), () => {
	hideMenu(menuMainNode);
	setActiveMenu(MENU_LEADERBOARD);
	updateLeaderboard();
})

handleClick($('.leader-board-close'), () => {
	hideMenu(menuMainNode);
	setActiveMenu(MENU_MAIN);
})

handleClick($('.login-btn'), () => {
	hideMenu(menuMainNode);
	setActiveMenu(MENU_LOGIN);
})

handleClick($('.logout-btn'), () => {
	...
	currentUser.innerHTML = `<h2>Guest</h2>`
	snackbar("Logged Out", '#00C851');
})

handleClick($('.login-close'), () => {
	hideMenu(menuMainNode);
	setActiveMenu(MENU_MAIN);
})
...
function updateLeaderboard() {
	...
	if (localStorage.getItem('userId')) {
		fetch(`/rank?userId=${localStorage.getItem("userId")}`)
			.then(response => response.json())
			.then(data => {
				rankele.innerHTML = `Your Rank ${data}`;
			})
			.catch(error => {
				snackbar('Error fetching rank', '#ff4444');
			});
	} else {
		rankele.innerHTML = '';
	}
	fetch('/leaderboard')
		...
		.then(data => {
			...
			data.forEach((player, index) => {
				...
				row.insertCell(1).textContent = player.userId; // Name
				row.insertCell(2).textContent = player.score; // Score
				row.insertCell(3).textContent = player.blocksSliced; // Blocks Sliced
			});
		})
		.catch(error => {
			snackbar('Error fetching leaderboard data', "#ff4444");
		});
}
...
// handleClick($('.play-leaderboard-btn'), () => {
// 	setActiveMenu(MENU_LEADERBOARD)
// })

// handleClick($('.play-login-btn'), () => {
// 	setActiveMenu(MENU_LOGIN)
// })
	...
	if (isNewHighScore()) {
		...
		if (localStorage.getItem('userId')) {
			const data = {
				...
				blocksSliced: state.game.cubeCount
			}
			const options = {
				method: 'POST',
				headers: {
			...
			fetch('/update', options)
				.then(response => response.json())
				.catch(error => {
					snackbar('Error while updating the score', '#ff4444');
				})
		}
	}
...
function snackbar(content, color = "#333") {
...
if (localStorage.getItem('userId')) {
	const currentUser = document.getElementById('current-user');
	currentUser.innerHTML = `<h2>${localStorage.getItem('userId')}</h2>`
	setLatestScore(localStorage.getItem('userId'))
}
	...
	fetch('/login', options)
		.then(response => {
			...
			if (response.ok) {
				...
				// If login is unsuccessful, handle the error
		...
		.then(token => {
			...
			snackbar(`logged in as ${userId}`, '#00C851');
			hideMenu(menuMainNode);
			setActiveMenu(MENU_MAIN);
			const loginBtn = document.querySelector('.login-btn');
			const logoutBtn = document.querySelector('.logout-btn');
		...
		.catch(error => {
			...
			snackbar('Login Failed', '#ff4444')
			// Display an error message or perform some other action
		});

	setLatestScore(userId)

}

function setLatestScore(userId) {
	fetch(`/score?userId=${userId}`)
		.then(response => {
			...
			localStorage.setItem(highScoreKey, body.score);
		}).catch(error => {
			snackbar('Error Fetching your score', '#ff4444');
		})
}

function signup() {
	// Get user ID and password from input fields
	var userId = document.getElementById('signup-userId').value;
	var password = document.getElementById('signup-password').value;
	const data = {
		userId: userId,
		password: password
	};
	if (!validSignupPassword && !validSignupUserId) {
		return;
	}
	...
	fetch('/signup', options)
		.then(response => {
			...
			if (response.ok) {
				...
				// If login is unsuccessful, handle the error
				snackbar('Signup Failed', '#ff4444')
				// Display an error message or perform some other action
			}
		}).then(data => {
			// Do something with the data
			if (data) {
				snackbar(`Signup Successful`, '#00C851')
			} else {
				snackbar(`User with ${userId} already exists`, '#ffbb33')
			}
		}).catch(error => {
			// If an error occurs during the request, handle it
			snackbar('Unable to login some internal error', '#ff4444')
			// Display an error message or perform some other action
		});

}

const userIdSignupInput = document.getElementById('signup-userId');
const passwordSignupInput = document.getElementById('signup-password');
const userIdArg = document.getElementById('userId-arg');
const passwordArg = document.getElementById('password-arg');
validSignupUserId = false;
validSignupPassword = false;

userIdSignupInput.addEventListener('input', () => {
	const userId = userIdSignupInput.value.trim();
	if (userId === '') {
		userIdSignupInput.style.borderWidth = '0';
		validSignupUserId = false;
		userIdArg.innerHTML = "userId cannot be null";
		return;
	}
	if (userId.length < 5) {
		userIdSignupInput.style.border = '2px solid #ff4444';
		validSignupUserId = false;
		userIdArg.innerHTML = "userId length cannot be less than 5";
		return;
	}

	fetch(`/unique?userId=${userId}`)
		.then(response => response.json())
		.then(data => {
			if (data) {
				userIdSignupInput.style.border = '2px solid #00C851';
				validSignupUserId = true;
				userIdArg.innerHTML = "";
			} else {
				userIdSignupInput.style.border = '2px solid #ff4444';
				userIdArg.innerHTML = "userId already taken"
				validSignupPassword = false;
			}
		})
		.catch(error => {
			console.log(error)
			snackbar("Error to validate userid", '#ffbb33');
		});
});
...
passwordSignupInput.addEventListener('input', () => {
	...
	if (password === '') {
		passwordSignupInput.style.borderWidth = '0';
		validSignupPassword = false;
		passwordArg.innerHTML="password length cannot be null";
		return
	} else if (password.length < 5) {
		passwordSignupInput.style.border = '2px solid #ff4444';
		validSignupPassword = false;
		passwordArg.innerHTML="password length cannot be less than 5";
		return
	} else {
		passwordSignupInput.style.border = '2px solid #00C851';
		validSignupPassword = true;
		passwordArg.innerHTML="";
	}
});
